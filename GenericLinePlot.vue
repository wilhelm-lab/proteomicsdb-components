...
export default {
  ...
  methods: {
    ...
    highlightMouseOver: function (sId, x, y) {
      d3.select(this.$el).select('#errorLine-' + sId)
      .attr('class', 'highlightLines');
      if (!this.hideLegend) {
        d3.select(this.$el).select('#legendLine-' + sId)
        .attr('class', 'highlightLines');
        d3.select(this.$el).select('#legendText-' + sId)
        .attr('font-weight', 'bold');
        d3.select(this.$el).selectAll('#dotsLegend-' + sId)
        .attr('transform', 'translate(' + 25 / 2 + ',' + 10 / 2 + ') scale(1)'); // sry for hard coding
      }
      d3.select(this.$el).select('#plotLine-' + sId)
      .attr('class', 'highlightLines');
      d3.select(this.$el).selectAll('#dots-' + sId)
    ...
    drawPlot: function (aData, aPoints, aProperties, aParameters) {
      ...
      var minDomain = this.kinobeads ? 0.1 : d3.min(aData, function(d) { return d3.min(d, function(e) {
          return e[0];
        });
      }) ; 
      ...
      .attr('x', width / 2)
      .attr('y', height + 30)
      .text(this.xAxisLabel !== null ? this.xAxisLabel : 'Treatment [' + aProperties[0].doseUnit + ']');
      ...
      .attr('y', -40)
      .attr('x', 0 - height / 2)
      .text(this.yAxisLabel !== null ? this.yAxisLabel : aProperties[0].responseUnit);
