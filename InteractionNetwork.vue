...
export default {
  ...
  methods: {
    ...
    prepareGraph: function prepareGraph(svg, AccessionId, sRelationTypeIds) {
      ...
      d3.json(
        ...
        .get(function (error, graph) {
          if (!error) {
            that.prepareGraphForSvgDefinitions(
              svg,
              graph,
              that.getResourceTypesInitially(graph.nodes)
            );
    ...
    direct2AccessionId: function direct2AccessionId(
      endPointAccession,
      protein_id
    ) {
      var that = this;
      d3.json(
        endPointAccession.replace("__parameter1__", protein_id),
        function (error, oAccessionId) {
          if (!error) {
    ...
    prepareGraphForSvgDefinitions: function prepareGraphForSvgDefinitions(
      ...
      d3.json(this.endpointMarker, function (error, data) {
        ...
        that.createV4SelectableForceDirectedGraph(
          svg,
          graph,
          aGlobalCheckedTypes
        );
    ...
    createV4SelectableForceDirectedGraph:
      function createV4SelectableForceDirectedGraph(
        svg,
        graph,
        aGlobalCheckedTypes
      ) {
