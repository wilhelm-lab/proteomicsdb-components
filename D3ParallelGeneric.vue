...
export default {
  ...
  methods: {
    fireKeyChange: function() {
      this.$emit('keyChange', this.selectedLines, this.selectedKey)
    },
    ...
    drawPlot: function () {    
      ...
      if (this.chartData && this.chartData.length > 0) {
          ...
          // prefill selectelLines will all visible lines
          that.selectedLines = '';
          ...
          var pc = ParCoords()(this.$el)
              ...
              .on('brushend', function(d) {
                  ...
                  that.selectedLines = modelids.length === 0 ? '-10' : modelids.reduce(function(prev, next) {
                      return prev + ';' + next;
                  });
                  that.fireKeyChange();
              }); // command line mode;

          var radioButtonGroup = d3.select(this.$el)
              .select('svg')
