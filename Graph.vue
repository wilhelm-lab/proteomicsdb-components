<template>
  <div id="pathwayContainer">
    <svg id="pathway"
      ...
      <rect width="2000" height="1200" fill="none" style="stroke-width: 2; stroke: lightgray"/>
      <!--         :viewBox="0 + ' ' + height / 4 + ' ' + width + ' ' + height">-->
      <defs>
        <marker id="activationMarker"
                viewBox="-0 -5 7 7"
                refX="3"
                refY="0"
                orient="auto"
        ...
        <marker id="otherInteractionMarker"
                viewBox="-0 -5 7 7"
                refX="3"
                refY="0"
                orient="auto"
                ...
                preserveAspectRatio="none"
                pointer-events="none">
          <path d="M 0,-1.5 L 3.5 ,0 L 0,1.5 Z" fill="white" stroke="#999" stroke-width="0.5pt"/>
        </marker>
        <marker id="inhibitionMarker"
                viewBox="-0 -5 7 7"
                refX="0.55"
                refY="0"
                orient="auto"
                ...
                preserveAspectRatio="none"
                pointer-events="none">
          <path d="M 0,-2.5 L 1.5,-2.5 L 1.5,2.5 L 0,2.5" fill="#999" stroke="none"/>
        </marker>
      </defs>
      <svg id="pathwayLegend" :width='legendWidth' :height='legendHeight' x=25 y=25></svg>
    </svg>
  </div>
</template>
...
export default {
  ...
  watch: {
    ...
    graphdataPTM: function () {
      ...
      d3.select("#pathway").transition().call(
          this.zoom.scaleTo,
    ...
    selectDownstreamButtonToggle: function () {
      ...
      */
      d3.select("#nodeG").selectAll("*").filter(d => d.selected).each(d => this.selectDownstreamNodes(d))
      this.onSelectedNodesChanged();
    },
  ...
  methods: {
    computeSkeletonGraph() {
      this.nodes = this.graphdataSkeleton.nodes.map(d => Object.create(d, {
        selected:
    ...
    computePTMGraph() {
      ...
      if (this.graphdataPTM.nodes) {
        this.nodes = this.nodes.concat(this.graphdataPTM.nodes.map(d => Object.create(d)))
        this.links = this.links.concat(this.graphdataPTM.links.map(d => Object.create(d)))
      }
    },
    renderGraph() {
    ...
    addReferences() {
      ...
      this.nodes.forEach(node => {
            ...
            if (node.type.includes("summary") && ["up", "down"].includes(node.regulation)) {
              node.ptmNodes = [];
    ...
    drawGraph() {
      ...
      const nodesSvg = nodeG
          ...
          .join("g")
          .attr("class", d => `node ${d.type} ${d.regulation ? d.regulation : ''}`)
      //TODO: Does it make sense to assign unique key?
      // .attr("key", d => d.id)

      //Draw each node as a rectangle
      nodesSvg.append("rect")
          .attr("class", d => `node-rect ${d.type} ${d.regulation ? d.regulation : ''}`)
      ...
      nodesSvg.append("text")
          .attr("class", d => `node-label ${d.type} ${d.regulation ? d.regulation : ''} `);
      nodesSvg.select(".node-label")
          .text(d => {
            if (d.label === null) return "";
            if (d.label.startsWith("TITLE:")) return d.label.substr(6).toUpperCase();
            if (d.type === "compound") return d.keggIds[0].split(":")[1];
            if (d.geneNames && d.geneNames.length > 1) return `${d.geneNames[0]}, ...`;
      ...
      nodesSvg.select(".node-rect")
          .attr("x", d => d.type === "ptm" ? -0.5 * graph.ptmNodeWidth : d.rectX)
          .attr("y", d => d.type === "ptm" ? -0.5 * graph.ptmNodeHeight : -graph.nodeHeight)
          .attr("width", d => d.type === "ptm" ? graph.ptmNodeWidth : d.rectWidth)
          .attr("height", d => d.type === "ptm" ? graph.ptmNodeHeight : graph.nodeHeight * 2)

      //Logic for node groups (KEGG sometimes subsumes multiple nodes in a group)
      graph.groupIds = new Set(graph.nodes.filter(d => d.groupId).map(d => d.groupId))
      groupG.selectAll(".group-path")
          .data(graph.groupIds)
          .join('g')
      ...
      linkG
          ...
          .attr("marker-end", d => {
            if (d.subtypes.includes("inhibition")) return 'url(#inhibitionMarker)';
            else if (d.subtypes.includes("binding/association")) return '';
            else if (d.subtypes.includes("activation")) return 'url(#activationMarker)';
            else return 'url(#otherInteractionMarker)';
          })
          .attr("stroke-dasharray",
              d => d.subtypes.includes("binding/association") ? "3 3" : undefined);
    ...
    addAnimation() {
      ...
      const simulation = d3.forceSimulation()
          ...
          .force("link", d3.forceLink(this.links)
              .strength(link => link.type.includes("ptmlink") ? PTM_LINK_FORCE_STRENGTH : 0)
              .distance(link => link.type.includes("ptmlink") ? link.target.textLength * PTM_LINK_FORCE_DISTANCE_MULTIPLIER : 0))
          //Collision repulsion Force
          .force("collide", d3.forceCollide().radius(
              node => node.type.includes("ptm") ? PTM_COLLISION_FORCE_RADIUS : 0
          ));
      ...
      simulation.nodes(graph.nodes).on("tick", function () {
        ...
        d3.select("#linkG").selectAll("line")
            .each(function (link) {
              ...
              let targetOffset = link.subtypes.includes("binding/association") ? 0 : 3.5
              link.sourceX = sourceX + Math.sin(angle) * graph.nodeHeight;
              link.targetX = targetX - Math.sin(angle) * (graph.nodeHeight + targetOffset);
              link.sourceY = link.source.y + Math.cos(angle) * graph.nodeHeight;
              link.targetY = link.target.y - Math.cos(angle) * (graph.nodeHeight + targetOffset);
            })
        ...
        d3.select("#linkG").selectAll(".edgepath").attr('d',
            edgepath => 'M ' + edgepath.source.x + ' ' + edgepath.source.y +
                ' L ' + edgepath.target.x + ' ' + edgepath.target.y)

        //Optionally rotate the edge labels, if they have been turned upside-down
        d3.select("#linkG").selectAll(".edgelabel").attr('transform', function (link) {
        ...
        function polygonGenerator(id) {
          let nodeCoords = d3.select("#nodeG")
              .selectAll("g")
              .filter(d => d.groupId === id)
              .data()
              .reduce((res, d) => res.concat([[d.leftX, d.y - graph.nodeHeight / 2], [d.leftX, d.y + graph.nodeHeight / 2],
                [d.rightX, d.y - graph.nodeHeight / 2], [d.rightX, d.y + graph.nodeHeight / 2]]), [])
          return d3.polygonHull(nodeCoords);
        }

        graph.groupIds.forEach(id => {
          let path = d3.select("#pathway").selectAll(".group-path:not(.legend)").filter(d => d === id)
          let centroid;
          path.select("path")
              ...
              .attr('d', groupPath => {
                ...
                return centroid ?
                    valueline(polygon.map(point => [point[0] - centroid[0], point[1] - centroid[1]])) :
                    null;
              })
          if (centroid) {
            d3.select(path.node()).attr("transform", `translate(${centroid[0]},${centroid[1]}) scale(1.33)`);
          }
        })
    ...
    enableZoomingAndPanning() {
      ...
      d3.select("#pathway").transition().duration(0).call(this.zoom.translateBy, 500, 0);
    },
    ...
    zoomed({transform}) {
      ...
      //Semantic zooms
      d3.select("#pathway").selectAll(".edgelabel:not(.legend)").attr("visibility", (transform.k < 1) ? "hidden" : "visible");
      d3.select("#pathway").selectAll(".ptm:not(.summary):not(.legend)").attr("display", (transform.k < 2) ? "none" : "block");
      d3.select("#pathway").selectAll(".ptm.summary:not(.unregulated):not(.legend)").attr("display", (transform.k > 2) ? "none" : "block");
    ...
    addTooltips() {
      ...
      const tooltip = d3.select("#pathwayContainer")
          .append("div")
          .attr("class", "tooltip")
          .attr("id", "nodetooltip")
      ...
      const mouseenter = function (e, node) {
        if (tooltip.attr("is-dragging") === "false") {
          ...
          if (d3.select(this).attr("class").includes("link")) {
            tooltip.html(`${node.type[0].toUpperCase() + node.type.slice(1)} (${node.subtypes})`)
            ...
            tooltip.html(`${node.label} ${regulationLabel} ${(+node.label > 1) ? 'Peptides' : 'Peptide'}`)
          } else {
    ...
    getPTMTooltipText(node) {
      return "<pre style='text-align: left'>" +
          "Regulation:        " + node.regulation + "<br>"
          + "Modified sequence: " + node.modifiedSequence + "<br>"
          + "Gene Name(s):      " + node.geneName + "<br>"
          + "Uniprot ID(s):     " + node.uniprotId + "<br>"
          + "pEC50 =            " + node.logEC50 + "<br>"
          + "|Effect| =         " + node.absEffect + "<br>"
          + "R^2 =              " + node.r2 + "<br>"
          + "Experiment:        " + node.experiment + "<br>"
          + "</pre>";
    },
    ...
    enableNodeSelection() {
      ...
      d3.select("#nodeG").selectAll("g")
          .on("click", (e, d) => {
            ...
            if (!e.ctrlKey) d3.select("#nodeG").selectAll("g").each(d => d.selected = false);
            ...
            d3.select("#pathway").selectAll(".ptmlink:not(.legend)").each(l => {
            ...
            if (d.type.includes("summary") && ["up", "down"].includes(d.regulation)) {
            ...
            if (d.type.includes("ptm") && !d.type.includes("summary") && !e.ctrlKey) {
      ...
      d3.select("#pathway")
          .on("click", () => {
            ...
            d3.select("#nodeG").selectAll("g").each(d => d.selected = true);
            this.$emit('update-ptm-infobox', undefined)
            this.onSelectedNodesChanged();
          })
    },
    ...
    selectDownstreamNodes(node) {
      ...
      d3.select("#linkG").selectAll("line").each(l => {
            ...
            else if (l.subtypes.includes("binding/association") && l.target === node && !l.source.selected) {
    ...
    highlightSelectedNodes() {
      ...
      d3.select("#nodeG").selectAll("g:not(.ptm)").filter(node => node.selected)
          .style("opacity", 1)
      ...
      d3.select("#linkG").selectAll("line").filter(link => link.source.selected && link.target.selected)
          .style("opacity", 1)
      ...
      d3.select("#pathway").selectAll(".edgelabel:not(.legend)").filter(link => link.source.selected && link.target.selected)
          .style("opacity", 1)
      ...
      d3.select("#pathway").selectAll(".ptm:not(.legend)").filter(ptmNode => {
      ...
      const selectedGroupIDs = this.nodes.filter(d => (d.selected && d.groupId)).map(d => d.groupId);

      d3.select("#groupG").selectAll("g").filter(d => selectedGroupIDs.includes(d))
          .style("opacity", 1)
    ...
    sendSelectedPTMsToParent() {
      const MAX_CURVES = 20;
      let selectedPTMNodes = this.nodes
          .filter(d => d.type === "ptm" && d.selected);
    ...
    updateNUnselected() {
      this.nUnselected = d3.select("#nodeG").selectAll("g").filter(d => !d.selected).size();
    },
    ...
    renderLegend() {
      ...
      svg.append("rect")
          .attr("x", 5).attr("y", 5)
          .attr("width", this.legendWidth - 10).attr("height", this.legendHeight - 10)
          .attr("fill", "white").style("stroke-width", "1.5").style("stroke", "darkgray")

      const xOffset = 25, yOffset = 40, lineHeight = 50, paragraphMargin = 15;

      svg.append("rect").attr("class", "node-rect gene legend")
          .attr("x", xOffset).attr("y", yOffset + lineHeight * 0 + paragraphMargin * 0)
          .attr("rx", 10).attr("ry", 30)
          .attr("width", 35).attr("height", 25)
      svg.append("text").attr("class", "legend")
          .text("Gene/Compound")
          .attr("x", xOffset + 40).attr("y", yOffset + lineHeight * 0 + paragraphMargin * 0)

      svg.append("rect").attr("class", "node-rect map legend")
          .attr("x", xOffset + 250).attr("y", yOffset + lineHeight * 0 + paragraphMargin * 0)
          .attr("rx", 10).attr("ry", 30)
          .attr("width", 35).attr("height", 25)
      svg.append("text").attr("class", "legend")
          .text("Pathway")
          .attr("x", xOffset + 290).attr("y", yOffset + lineHeight * 0 + paragraphMargin * 0)

      svg.append("rect").attr("class", "node-rect group-path legend")
          .attr("x", xOffset).attr("y", yOffset + lineHeight * 1 + paragraphMargin * 0)
          .attr("rx", 10).attr("ry", 30)
          .attr("width", 35).attr("height", 25)
      svg.append("text").attr("class", "legend")
          .text("Group")
          .attr("x", xOffset + 40).attr("y", yOffset + lineHeight * 1 + paragraphMargin * 0)

      svg.append("line").attr("class", "link legend")
          .attr("x1", xOffset).attr("y1", yOffset + lineHeight * 2 + paragraphMargin * 1)
          .attr("x2", xOffset + 65).attr("y2", yOffset + lineHeight * 2 + paragraphMargin * 1)
          .attr("marker-end", 'url(#activationMarker)');
      svg.append("text").attr("class", "legend")
          .text("Activation")
          .attr("x", xOffset + 80).attr("y", yOffset + lineHeight * 2 + paragraphMargin * 1)

      svg.append("line").attr("class", "link legend")
          .attr("x1", xOffset + 200).attr("y1", yOffset + lineHeight * 2 + paragraphMargin * 1)
          .attr("x2", xOffset + 265).attr("y2", yOffset + lineHeight * 2 + paragraphMargin * 1)
          .attr("marker-end", 'url(#inhibitionMarker)');
      svg.append("text").attr("class", "legend")
          .text("Inhibition")
          .attr("x", xOffset + 280).attr("y", yOffset + lineHeight * 2 + paragraphMargin * 1)

      svg.append("line").attr("class", "link legend")
          .attr("x1", xOffset).attr("y1", yOffset + lineHeight * 3 + paragraphMargin * 1)
          .attr("x2", xOffset + 65).attr("y2", yOffset + lineHeight * 3 + paragraphMargin * 1)
          .attr("marker-end", 'url(#otherInteractionMarker)');
      svg.append("text").attr("class", "legend")
          .text("Other Interaction")
          .attr("x", xOffset + 80).attr("y", yOffset + lineHeight * 3 + paragraphMargin * 1)

      svg.append("line").attr("class", "link legend")
          .attr("x1", xOffset).attr("y1", yOffset + lineHeight * 4 + paragraphMargin * 1)
          .attr("x2", xOffset + 65).attr("y2", yOffset + lineHeight * 4 + paragraphMargin * 1)
          .attr("stroke-dasharray", "10 10")
          .style("stroke-width", 10)
      svg.append("text").attr("class", "legend")
          .text("Binding/Association")
          .attr("x", xOffset + 80).attr("y", yOffset + lineHeight * 4 + paragraphMargin * 1)

      svg.append("rect").attr("class", "node-rect ptm up legend")
          .attr("x", xOffset).attr("y", yOffset + lineHeight * 5 + paragraphMargin * 2)
      ...
      svg.append("text").attr("class", "legend")
          .text("Up-")
          .attr("x", xOffset + 30).attr("y", yOffset + lineHeight * 5 + paragraphMargin * 2);

      svg.append("rect").attr("class", "node-rect ptm down legend")
          .attr("x", xOffset + 85).attr("y", yOffset + lineHeight * 5 + paragraphMargin * 2)
          .attr("rx", 25).attr("ry", 25)
          .attr("width", 25).attr("height", 25);
      ...
      svg.append("text").attr("class", "legend")
          .text("Down-")
          .attr("x", xOffset + 115).attr("y", yOffset + lineHeight * 5 + paragraphMargin * 2);

      svg.append("rect").attr("class", "node-rect ptm unregulated legend")
          .attr("x", xOffset + 200).attr("y", yOffset + lineHeight * 5 + paragraphMargin * 2)
          .attr("rx", 25).attr("ry", 25)
          .attr("width", 25).attr("height", 25);

      svg.append("text").attr("class", "legend")
          .text("Not regulated")
          .attr("x", xOffset + 230).attr("y", yOffset + lineHeight * 5 + paragraphMargin * 2);
