<template>
  <v-container fluid>
    <v-row>
      <div class="spectrumViewerClass" id = "spectrumViewerId">
      </div>
    </v-row>
  </v-container>
</template>
...
export default {
  ...
  methods: {
    ...
    clearOldLines: function () {
      var dataset = d3.select('#annotationContainer').selectAll(".line").data([]);
      dataset.exit().remove();
    },
    drawMassError: function() {
      var that = this;
      var x, y, xAxis, yAxis, dummyXAxis, dummyYAxis, massErrorLabel, shiftFactor, options = this.getOptions(), xValues = this.getMassErrorX(), 
      yValues = this.getMassError(), colors = this.getColors(), settings = this.getSettings(), labels = this.getLabels(),
      sequence = this.getSequence(), theoMz = this.getTheoreticalMz(), neutralLosses = this.getNeutralLosses(), sequenceBottom = this.getSequenceBottom();
      ...
      if (xValues && yValues) {
        ...
        massErrorLabel = this.fragmentContainer.selectAll(".yAnnotationLabel").text("Error (" + massErrorLabel + ")");
        ...
        this.fragmentContainer.selectAll("g.xAnnotation_2").attr("transform", "translate(0, " + options.fragments.height + ")").call(dummyXAxis);
        this.fragmentContainer.selectAll("g.yAnnotation_2").attr("transform", "translate(" + options.fragments.width + ",0)").call(dummyYAxis);

        // bind plotData to the chart to be rendered into circles by D3
        this.circleDataset = this.massErrorContainer.selectAll(".masserror").data(plotData);
        this.circleDataset.enter().append("circle").attr("class", "masserror");

        // actually render the circles and transition them in
        this.circleDataset.attr("cy", function () {
          return y(0);
        ...
        // draw the zero line
        var dataset = this.massErrorContainer.selectAll(".zeroline").data(baseLine);
        ...
        this.circleDataset.on("mouseenter", function(d, i) {
          ...
          let fitting_top_peak = that.plotContainer.selectAll(".bar")
          .filter(function(e){
            return e.id == d.top_id;

          });
          let fitting_bottom_peak = that.plotContainer2.selectAll(".bar")
          .filter(function(e){
          ...
          intensity_diff =  ((fitting_top_peak.data()[0]===undefined) || ( fitting_bottom_peak.data()[0]===undefined)) ? (fitting_top_peak.data()[0]===undefined ? d3.format(",.4f")(fitting_bottom_peak.data()[0].percentBasePeak) : d3.format(",.4f")(fitting_top_peak.data()[0].percentBasePeak)) : Math.abs(d3.format(",.4f")((fitting_top_peak.data()[0].percentBasePeak - fitting_bottom_peak.data()[0].percentBasePeak) ));
          ...
          var labelObj = that.plotContainer.selectAll(".barlabel").filter(function (e, j) { 
          ...
          var interactiveTitleObjects = that.titleContainer.selectAll("text").data(sequence);
          ...
          that.circleDataset.style("r", function(e, j) {
        ...
        that.circleDataset.on("mouseleave", function(d, i) {
          ...
          that.titleContainer.selectAll("text").data(sequence).style("fill", "black").style("stroke", "none");
          that.titleContainerBottom.selectAll("text").data(sequenceBottom).style("fill", "black").style("stroke", "none");
          let fitting_top_peak = that.plotContainer.selectAll(".bar")
          .filter(function(e){
            // return e.id == d.bottom_id;
            return e.id == d.top_id;
          ...
          that.circleDataset.style("r", function(data) {
            return data.radius;
    ...
    drawAnnotation: function() {
      var that = this;
      let x, y, y2, xAxis,xTopAxis, fragxAxis, yAxis,yAxis2, barDataset,barDataset2, labelDataset,labelDataset2, 
      options = this.getOptions(), 
      xValues = this.getX(), yValues = this.getIntensities(), 
      percentBasePeak = this.getPercentBasePeak(),
      xValues2 = this.getMirrorX(), yValues2 = this.getMirrorIntensities(), 
      percentBasePeak2 = this.getMirrorPercentBasePeak(),
      labels2 = this.getMirrorLabels(), labelCharges2 = this.getMirrorLabelCharges(),
      colors2 = this.getMirrorColors(),
      id = this.getId(),
      id2 = this.getIdMirror(),
      massError = this.getMassError(), colors = this.getColors(), labels = this.getLabels(), labelCharges = this.getLabelCharges(), 
      neutralLosses = this.getNeutralLosses(), widths = this.getWidths(),widths2 = this.getMirrorWidths(), sequence = this.getSequence();
      ...
      if (xValues && yValues) {
        ...
        for (let i = 0; i < xValues.length; i++) {
          ...
          label = this.formatLabel(label, neutralLoss, charge, this.ionizationMode);
        ...
        this.container.selectAll("g.xAnnotation").attr("transform", "translate(0, " + options.annotation.height + ")").call(xAxis);
        ...
        // bind the plotData variable to potential svg rectangle elements.
        barDataset2 = this.plotContainer2.selectAll(".bar").data(mirrorPlotData);
        // bind the plotData variable to potential svg text elements.
        labelDataset2 = this.plotContainer2.selectAll(".barlabel").data(mirrorPlotData);
        // bind the plotData variable to potential svg path elements.
        var lineDataset2 = this.plotContainer2.selectAll(".line").data(mirrorPlotData);
        ...
        barDataset.exit().attr("y", function() {
          return y(-10);
        }).attr("x", function(d) {
          return x(d3.min(xValues)) - d.width / 2;
        }).remove();
        // remove unneeded bars
        barDataset2.exit().attr("y", function() {
          return y(-10);
        }).attr("x", function(d) {
        ...
        labelDataset.exit().attr("y", function() {
          return y(d3.min(yValues));
        }).remove();
        // transition out unused labels
        labelDataset2.exit().attr("y", function() {
        ...
        labelDataset.on("mouseenter", function(d, i) {
          ...
          var label = labels[i];
          var fragmentType = label.indexOf("_") === -1 ? label.charAt(0) : label.slice(0,2);
          var fragmentNumber = label.indexOf("_") === -1 ? label.slice(1) : label.slice(2);
          var color = colors[i];
          ...
          if (fragmentType === "a" || fragmentType === "b" || fragmentType === "c" || fragmentType === "C" || fragmentType === "yb" ) {
            interactiveTitleObjects.style("fill", function(d, i) {
          ...
          var massErrorCircles = that.massErrorContainer.selectAll(".masserror");

          // make it a little bigger
          massErrorCircles.style("r", function(e) {
            if (d.id === e.top_id) {
              return 7;
            }
            // give it a stroke
        ...
        labelDataset2.on("mouseenter", function(d, i) {
          ...
          that.tip.html(function () {
            ...
            + "<strong style='font-style:italic;'>m/z:</strong> <span style='color:red'>" + d3.format(",.4f")(d.mz) + " </span><br><br>"
            + "<strong>Relative Abundance:</strong> <span style='color:red'>" + d3.format("0.2f")(d.percentBasePeak) + "%</span><br><br>"
            + "<strong>% TIC:</strong> <span style='color:red'>" + d3.format("0.2%")(d.intensity / TIC2) + "</span>";
          });
          // show tooltip
          that.tip.show(d,this);
    ...
    drawOriginSummary: function (topSpectrum = true) {
      var statisticsData = [];
      statisticsData.push({ title: "Origin: ", data: this.getModifications2(topSpectrum)});

      var dataset;
      if(topSpectrum){
        dataset = this.originContainer.selectAll(".origincategory").data(statisticsData);
        dataset.enter().append("text").attr("class", "origincategory");
      } else {
        dataset = this.originContainerBottom.selectAll(".origincategoryBottom").data(statisticsData);
        dataset.enter().append("text").attr("class", "origincategoryBottom");
      }
      // From line 1067 to 1086 we write the actual numerical data
      if (topSpectrum) {
        dataset = this.originContainer.selectAll(".origindata").data(statisticsData);
        dataset.enter().append("text").attr("class", "origindata origin");
      } else {
        dataset = this.originContainerBottom.selectAll(".origindataBottom").data(statisticsData);
    ...
    drawCorrelationSummary: function (topSpectrum = true) {
      ...
      var dataset;
      if(topSpectrum){
        dataset = this.statisticsContainer.selectAll(".precursorstatscategory").data(statisticsData);
        dataset.enter().append("text").attr("class", "precursorstatscategory");
      } else {
        dataset = this.statisticsContainerBottom.selectAll(".precursorstatscategoryBottom").data(statisticsData);
      ...
      // From line 1067 to 1086 we write the actual numerical data
      if (topSpectrum) {
        dataset = this.statisticsContainer.selectAll(".precursorstatsdata").data(statisticsData);
        dataset.enter().append("text").attr("class", "precursorstatsdata");
      } else {
        dataset = this.statisticsContainerBottom.selectAll(".precursorstatsdataBottom").data(statisticsData);
    ...
    drawPrecursorSummary: function() {
      ...
      summaryData.push({title: "Charge: ", data: this.ionizationMode + Math.abs(charge)});
      summaryData.push({ title: "Fragmented Bonds: ", data: this.getFragmentedBonds(fragments, sequence.length) });
      var summaryDataBottom = [];
      summaryDataBottom.push({title: "Precursor m/z: ", data: d3.format(",.4f")(precursorMzBottom)});
      summaryDataBottom.push({title: "Charge: ", data: this.ionizationMode + Math.abs(chargeBottom)});
      summaryDataBottom.push({ title: "Fragmented Bonds: ", data: this.getFragmentedBonds(fragmentsBottom, sequenceBottom.length)});


      // From line 1047 to 1065 we write the Title text from the summary data objects e.g. "Precursor m/z"
      var dataset = this.peptideContainer.selectAll(".precursorstatscategory").data(summaryData);
      var datasetBottom = this.peptideContainerBottom.selectAll(".precursorstatscategoryBottom").data(summaryDataBottom);
      ...
      // From line 1067 to 1086 we write the actual numerical data
      dataset = this.peptideContainer.selectAll(".precursorstatsdata").data(summaryData);
      datasetBottom = this.peptideContainerBottom.selectAll(".precursorstatsdataBottom").data(summaryDataBottom);
    ...
    drawInteractiveTitle: function() {
      var x, y, dataset, options = this.getOptions(), sequence = this.getSequence(), aminoAcidTicks = this.getTickData(), mods = this.getModifications();
    ...
    drawInteractiveTitleBottom: function() {
      var x, y, dataset, options = this.getOptions(), sequence = this.getSequenceBottom(), aminoAcidTicks = this.getTickData(false), mods = this.getModificationsBottom();
      // set variables to plot interactive title
      ...
      // prepare to add individual text element for each character in the peptide sequence. The class is called "aminoacid".
      dataset = this.titleContainerBottom.selectAll(".aminoacid").data(sequence);
      dataset.enter().append("text").attr("class", "aminoacid");
      ...
      // add the lines
      dataset = this.titleContainerBottom.selectAll(".line").data(aminoAcidTicks);
    ...
    drawLine: function(x, y, d) {
      ...
      d3.select('#plotContainer').selectAll(".line").attr("d", function(d) {
    ...
    initialize: function() {
      ...
      this.titleContainerBottom = this.svg.append("g").attr("id", "titleContainerBottom");

      // svg element to show summary data about peptide
      this.peptideContainer = this.svg.append("g").attr("id", "peptideContainer");
      this.peptideContainerBottom = this.svg.append("g").attr("id", "peptideContainerBottom");
      // svg element to show correlation info about peptide
      this.statisticsContainer = this.svg.append("g").attr("id", "statisticsContainer");
      this.statisticsContainerBottom = this.svg.append("g").attr("id", "statisticsContainerBottom");

      // svg element to show origin of spectrum
      this.originContainer = this.svg.append("g").attr("id", "originContainer");
      this.originContainerBottom = this.svg.append("g").attr("id", "originContainerBottom");
      ...
      this.fragmentContainer = this.svg.append("g").attr("id", "fragmentContainer");
      // main svg container to hold spectrum annotations
      this.container2 = this.svg.append("g").attr("id", "bottomSpectrumContainer");


      // invisible rectangle on Y axis used to catch zoom events
      this.zoomY = this.container.append("rect")
      .attr("id", "yZoom")
      .attr("fill", "none")
      ...
      this.zoomX = this.container.append("rect")
      .attr("id", "xZoom")
      .attr("fill", "none")
      .attr("x", "0")
      ...
      // invisible rectangle on Y axis used to catch zoom events
      this.zoomY2 = this.container2.append("rect")
      .attr("id", "yZoom2")
      .attr("fill", "none")
      .attr("x", -options.annotation.margin.left)
      .attr("y", "0")
      ...
      // invisible rectangle on X axis used to catch zoom events
      this.zoomX2 = this.container2.append("rect")
      .attr("id", "xZoom2")
      .attr("fill", "none")
      .attr("x", "0")
      ...
      this.plotContainer = this.container.append("g").attr("id", "annotationContainer");
      // container to hold annotated spectrum
      this.plotContainer2 = this.container2.append("g").attr("id", "annotationContainer2");

      // container to hold mass error dot plot
      this.massErrorContainer = this.fragmentContainer.append("g").attr("id", "massErrorContainer");
      ...
      this.plotContainer.append("clipPath")
      .attr("id", "clippy")
      .append("rect")
      .attr("x", "0")
      .attr("y", "0")
      ...
      //TODO check later
      // place a clip mask over the annotated spectrum container to prevent svg elements from displaying out of the SVG when zooming. 
      this.plotContainer2.append("clipPath")
      .attr("id", "clippy2")
      .append("rect")
      .attr("x", "0")
      .attr("y", "0")
      ...
      this.fragmentContainer.attr("transform", "translate(" + options.fragments.margin.left + "," + (options.offsets.middleOffset) + ")");

      // Define mass error chart x-axis
      this.fragmentContainer.append("g")
      .attr("class", "xAnnotation")
      ...
      this.massErrorContainer.append("clipPath")
      .attr("id", "clippy2")
      .append("rect")
      .attr("x", "0")
      .attr("y", "0")
      ...
      this.container.attr("transform", "translate(" + options.annotation.margin.left + ", " + (options.annotation.margin.top) +  ")");

      // bind the clip path to the annotated mass spectrum
      this.plotContainer.attr("clip-path", "url(#clippy)");

      // // position the container which will hold the marked peptide sequence
      this.container2.attr("transform", "translate(" + options.annotation.margin.left + ", " + (options.annotation.margin.top + options.offsets.bottomOffset) +  ")");

      // bind the clip path to the annotated mass spectrum
      this.plotContainer2.attr("clip-path", "url(#clippy)");

      // position the container which will hold the marked peptide sequence
      this.titleContainer.attr("transform", "translate(" + options.interactiveTitle.margin.left + ", " + options.interactiveTitle.margin.top + ")");
      this.titleContainerBottom.attr("transform", "translate(" + options.interactiveTitleBottom.margin.left + ", " + options.interactiveTitleBottom.margin.top + ")");

      this.peptideContainer.attr("transform", "translate(" + (options.statistics.margin.left + options.statistics.width / 2) + ", " + options.statistics.margin.top + ")");
      this.peptideContainerBottom.attr("transform", "translate(" + (options.statisticsBottom.margin.left + options.statisticsBottom.width / 2) + ", " + options.statisticsBottom.margin.top + ")");
      this.statisticsContainer.attr("transform", "translate(" + (options.statistics.margin.left + options.statistics.width / 2) + ", " + options.statistics.margin.top + ")");
      this.statisticsContainerBottom.attr("transform", "translate(" + (options.statisticsBottom.margin.left + options.statisticsBottom.width / 2) + ", " + options.statisticsBottom.margin.top + ")");

      this.originContainer.attr("transform", "translate(" + (options.statistics.margin.left + options.statistics.width / 2) + ", " + options.statistics.margin.top + ")");
      this.originContainerBottom.attr("transform", "translate(" + (options.statisticsBottom.margin.left + options.statisticsBottom.width / 2) + ", " + options.statisticsBottom.margin.top + ")");
...
getTickData: function(topSpectrum = true) {
  ...
  var length = topSpectrum ? this.getSequence().length : this.getSequenceBottom();
  var labelObj;
  var i;
  var tempLabel;
  ...
  for (i = 0; i < labels.length; i++) {
    tempLabel = labels[i];
    if (tempLabel.charAt(0) == "[" || tempLabel.indexOf('IM') !== -1 || tempLabel.indexOf('yb') !== -1) {
      continue;
    }
    if (tempLabel) {
      labelObj = 
...
setSvgSize: function() {
  ...
  d3.select('.chart').attr('viewBox','0 0 '+ (options.svg.width + options.svg.margin.left + options.svg.margin.right) + ' ' +
...
formatLabel: function(label, neutralLoss, charge, ionizationMode) {
  ...
  if (label) {
    ...
    if (label[0] == "[") {
      ...
      // [M+2H-H₂O]⁺²
      returnString += "[" + label + this.formatNeutralLoss(neutralLoss) + "]";
      ...
      if (ionizationMode == "+") {
        ...
        if (charge == 1) {
          ...
          if (label[0] == "C") {
            ...
            returnString += "-H" + this.formatNeutralLoss(neutralLoss) + "]";
            // labels for z+1 ions
          } else if (label[0] == "Z") {
            returnString += "[z";
            for (i = 0; i < fragmentNumber.length; i++) {
            ...
            returnString += label.indexOf("_") === -1 ? label[0] : label.slice(0,2);
            // add a unicode bullet to z-ions to support etd fragmentation
            if (label[0] == "z") {
              returnString += "\u2022";
            }
