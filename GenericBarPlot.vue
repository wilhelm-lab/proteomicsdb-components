<template>
  <div>
    ...
    <v-btn x-small class="barPlotClearSelectionButton" 
        v-if="multiSelection" 
        @click="clearSelectedBars" 
        :disabled="!this.selectedModelIds || this.selectedModelIds.data.length == 0">
      Clear selection
    </v-btn>
  </div>
</template>
...
export default {
  props: {
    ...
    sSelectedModelIds: { // protein selected through Dropdown
      type: Object,
      default: undefined
    },
    title: {
  ...
  methods: {
    ...
    drawPlot: function (oData) {
      ...
      var width = Math.max(oData.data.length * this.barWidth, d3.select(this.$el).attr('width'))
      ...
      var bar = bars.selectAll('g')
        .data(oData.data)
        .enter().append('g')
        .attr('transform', (d, i) => { return 'translate(' + (i * this.barWidth + 1) + ', 0)' })
      ...
      this.data.chartObjects.bars = bar.append('rect')
        .attr('class', function () {
          return 'Bar'
        })
      ...
      bar.append('rect')
        ...
        .on('click', function (clickEvent, f) {
          ...
          if (oSelectedModelIds) {
            ...
            if (aSelectedModelIds.some(function (modelIdsEntry) {
              return modelIdsEntry === f.ModelId
            })) {
              d3.selectAll('.Bar').filter(function (e) {
                return e.ModelId === f.ModelId
              }).attr('class', 'Bar')
              
              d3.selectAll('.BackgroundBar').filter(function (e) {
                return e.ModelId === f.ModelId
              }).attr('class', 'BackgroundBar')
              
              that.selectedModelIds = { // remove it
              ...
              that.selectedModelIds = { // adding stuff
    ...
    getLegendStyleClass: function (i) {
      var aClass = ['BetterValue', 'SelectedModelIds', 'AtMostTenfold', 'MoreThanTenfold']
      return aClass[i]
    },
