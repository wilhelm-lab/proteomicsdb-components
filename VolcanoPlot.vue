...
export default {
  ...
  methods: {
    drawPlot: function (data) {
      ...
      var svg = d3v4.select(this.$el).append('svg')
      ...
      var iAbsoluteFoldChange = d3v4.max([Math.abs(oMinMaxPvalueFoldChange.fold_min), Math.abs(oMinMaxPvalueFoldChange.fold_max)])
      ...
      var yScale = d3v4.scaleLinear()
        .domain([oMinMaxPvalueFoldChange.p_min * 0.95, oMinMaxPvalueFoldChange.p_max * 1.05])
      ...
      var fMidPValue = (oMinMaxPvalueFoldChange.p_max + oMinMaxPvalueFoldChange.p_min) / 2
      g.append('text')
        .attr('transform', 'translate(' + -37 + ' ,' +
              yScale(fMidPValue) + ') rotate(-90)')
        .style('text-anchor', 'middle')
        .text(this.yAxis)
      ...
      function getSelectedKeys () {
        that.highlightCircle(d3v4.selectAll('circle').data().map((m) => { return m.GeneName }), false)
        that.$emit('selection-wrapper', {
          aSelectedGeneNamesUp: d3v4.selectAll('.active').data().map(function m (d) {
            if (d.x >= 0) { return { genename: d.GeneName, y: d.y, x: d.x } } return null
          }).filter(function f (d) { return d !== null }),
          aSelectedGeneNamesDown: d3v4.selectAll('.active').data().map(function m (d) {
            if (d.x < 0) { return { genename: d.GeneName, y: d.y, x: d.x } } return null
          }).filter(function f (d) { return d !== null })
        })
      }
      ...
      function brushmoved () {
        ...
        if (s === null) {
          ...
          circle.classed('active', function c (d) {
            return (sxy[0][0] <= d.x && // correct area of plot
                      sxy[1][0] >= d.x &&
                      sxy[0][1] >= d.y &&
                      sxy[1][1] <= d.y) ||
      ...
      function highlightCircle (aGeneNames, bFlag) {
        ...
        if (bFlag) {
          d3v4.selectAll('circle').filter(function f (d) {
            return aGeneNames.includes(d.GeneName)
          }).transition(t).attr('r', 10)
        } else {
          d3v4.selectAll('circle').filter(function f (d) {
            return aGeneNames.includes(d.GeneName)
          }).transition(t).attr('r', 3.5)
        }
      }

      function unSelectCircles (aGeneNames) {
        d3v4.selectAll('.active').filter(function f (d) {
          return aGeneNames.includes(d.GeneName)
    ...
    minMaxXY: function minMaxXY (data) {
      var out = {}
      out.p_min = d3v4.min(data.map(function m (d) {
        return d.y
      }))
      out.p_max = d3v4.max(data.map(function m (d) {
        return d.y
      }))
      out.fold_min = d3v4.min(data.map(function m (d) {
        return d.x
      }))
      out.fold_max = d3v4.max(data.map(function m (d) {
        return d.x
      }))
