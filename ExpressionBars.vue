<template>
  <div>
  <div id="expressionBarsAxisTop"
       class="expressionBarsAxisTop"
       >
  </div>
  <v-card id="expressionBarsChart"
       class="expressionBarsChart"
       style="max-height:650px;overflow-y:auto;"
       flat
       >
  </v-card>
  <div id="expressionBarsAxisBottom"
       class="expressionBarsAxisBottom"
       >
  </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    redraw: function(){
      ...
      var svg = d3.select(this.$el).select('.expressionBarsChart').append('svg')
      .attr('class', 'ExpressionBars')
      .attr('width', width + margin.left + margin.right)
      .attr('height', oBarAttributes.height * oData.length + margin.chart.top + margin.bottom);
      var content = svg.append('g')
      .attr('transform', 'translate(' + margin.left + ',' + margin.chart.top + ')')
      .append('g').attr('style', 'overflow:auto;');

      this.svg = svg;

      var xMaxBasedOnMaxNormalizedIntensity = d3.max(oData, function getMaxNormIntensity(d) {
        return d.MAX_NORMALIZED_INTENSITY;
      });
      var xMaxBasedOnNormalizedIntensity = d3.max(oData, function getMaxOfNormIntensity(d) {
        return oBarAttributes.width(d);
      });
      var xMax = d3.max([xMaxBasedOnMaxNormalizedIntensity, xMaxBasedOnNormalizedIntensity]); // for ratios no MAX_NORMALIZED_INTENSITIES are given

      var xMinBasedOnMinNormalizedIntensity = d3.min(oData, function getMinNormIntensity(d) {
        return d.MIN_NORMALIZED_INTENSITY;
      });
      var xMinBasedOnNormalizedIntensity = d3.min(oData, function getMinOfNormIntensity(d) {
        return oBarAttributes.width(d);
      });
      var xMin = d3.min([xMinBasedOnNormalizedIntensity, xMinBasedOnMinNormalizedIntensity]); // for ratios no MAX_NORMALIZED_INTENSITIES are given
      ...
      var xScale;
      if (xMin > 0) {
        xScale = d3.scaleLinear().domain([0, xMax]).range([0, oSvgAttributes.width]);
      } else {
        if (xMax > 0) {
          xScale = d3.scaleLinear().domain([xMin, xMax]).range([0, oSvgAttributes.width]);
        } else {
          xScale = d3.scaleLinear().domain([xMin, 0]).range([0, oSvgAttributes.width]);
        }
      }
      ...
      var xAxisTopSvg = d3.select(this.$el).select('#expressionBarsAxisTop').append('svg')
      .attr('width', width + margin.left + margin.right)
      ...
      var xAxisBottomSvg = d3.select(this.$el).select('#expressionBarsAxisBottom').append('svg')
      .attr('width', width + margin.left + margin.right)
      .attr('height', margin.top+margin.chart.top + 5)
      .append('g');
      ...
      if (xAxisDomBottom.empty()) {
        xAxisDomBottom = xAxisBottomSvg.append("g")
        .attr("class", "x axis")
      }
      xAxisDomBottom
      ...
      // redraw
      var groups = content.selectAll('g.bar').data(oData, function getBarAttributesId(d) {
        return oBarAttributes.id(d);
      });

      // add elements
      var g = groups.enter().append('g').classed('bar', true).classed('clickEvent', this._bClickEvent);
      ...
      svg.selectAll('g.y_axis text')
      .style('font-family', 'sans-serif')
      .style('font-size', '11px');

      svg.selectAll('g.y_axis text.label')
      .style('font-family', 'sans-serif')
      ...
      .attr('barId', function(d) { return(d.SAP_SYNONYM)})
      ...
      svg.selectAll('g.bar').selectAll('line.pointer')
      .transition()
      .duration(1000)
      ...
      svg.selectAll('g.bar').selectAll('line.end_left')
      .transition()
      .duration(1000)
      ...
      svg.selectAll('g.bar').selectAll('line.end_right')
      .transition()
      .duration(1000)
      ...
      // update tipsy
      //if (oBarAttributes.tipsy) {
        //  $(sBarsId + ' .bar rect').tipsy({
          //    gravity: 'sw',
          //    html: true,
          //    title: function getBarTooltip() {
            //      var d = this.__data__;
            //      return oBarAttributes.tipsy(d);
          //    }
        //  });
      //}
    ...
    toggleBarSampleId: function(element) {
      var bar = d3.select(this.$el).selectAll('g.bar rect').filter(d => d.SAMPLE_ID === element.SAMPLE_ID);
      this.toggleBarFinal(bar);
    },
    toggleBarTissueId: function(element) {
      var bar = d3.select(this.$el).selectAll('g.bar rect').filter(d => d.TISSUE_ID === element.TISSUE_ID);
      this.toggleBarFinal(bar);
    },
